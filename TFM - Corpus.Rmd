...
fdatos <- data.frame()
file.names <- dir(nombreruta, pattern = "ANSII.csv")
...
#nube de palabras
wordcloud(palab_frec.df$word, palab_frec.df$freq, random.color=FALSE,random.order = FALSE, colors=colorRampPalette(brewer.pal(6,"Blues"))(32),
...
# Palabras especificas
# revisar, añadirlas a un fichero
doc.corpus.dec <- tm_map(doc.corpus.dec, removeWords, c("vomito", "enfermedad","sindrome","asociacion","marfan"))   
...
wordcloud(palab_frec_dec.df$word, palab_frec_dec.df$freq, random.color=FALSE,random.order = FALSE, colors=colorRampPalette(brewer.pal(6,"Blues"))(32),
max.words=45, rot.per=0)
...
#palab_frec.df$word <- as.character(palab_frec.df$word)
#palab_frec.df$tipo <- "Facebook"
#corpus_1 <- corpus(palab_frec.df,textField="word")

#palab_frec_dec.df$word <-as.character(palab_frec_dec.df$word)
#palab_frec_dec.df$tipo <- "Decalogo"
#corpus_2 <- corpus(palab_frec_dec.df,textField="word")

#docvars(corpus_1, "id") <- "Facebook" 
#docvars(corpus_2, "id") <- "Decalogo"

#corpus_unido <- corpus_1 + corpus_2

#TDM_unido <- TermDocumentMatrix(as.VCorpus(corpus_unido), 
#       control = list(wordLengths = c(3, Inf))) 
#matriz_unido <- dfm(corpus_unido, groups = "id")

#plot(TDM_unido, comparison=TRUE)

#matriz_unido_tr <- t(as.matrix(matriz_unido))

comparison.cloud(unido_all_mat,scale=c(0.5,1.5),max.words=45,
