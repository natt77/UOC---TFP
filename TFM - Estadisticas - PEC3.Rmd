...
Vamos a crear 3 nubes de palabras, una para cada nivel: alto, medio y bajo, para visualizar las diferencias entre ellos.
```{r}
...
Mediante esta visualización no podemos obtener demasiada información, busquemos mejor graficos que incluyan la frecuenca.
...
#Modelo que precide la polaridad en función del día de la semana y la hora
mod <- lm(datos_sent$polarity ~ datos_sent$dia_sem + datos_sent$hora)
...
#Modelo que precide en engagement en función del día de la semana y la hora del día
mod1 <- lm(datos_sent$engagement_fb ~ datos_sent$dia_sem + datos_sent$hora)
...
Estos graficos nos confirman lo que habíamos visto con los modelos, el engagement no parece afectado por la hora o el día de la semana, pero la polaridad de los mensajes si está influenciada por la hora y el dia.
