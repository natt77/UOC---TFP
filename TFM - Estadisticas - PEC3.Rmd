...
fsentim <-  read.csv("/resources/Sentiment_output.csv", header = TRUE, stringsAsFactors = FALSE,encoding = "ANSII", sep=";")
...
Calculamos ahora las correlaciones entre la polaridad y 

```{r}
cor.test(datos_sent$polarity, datos_sent$likes_count_fb)

modelo_like <- lm(datos_sent$polarity~datos_sent$likes_count_fb)
...
Tenemos un P-valor muy alto 0.3176, lo que nos lleva a aceptar la hipótesis nula que nos indica que las variables no están correlacionadas.

Voy a repetirlo, pero quitando los casos con polaridad=0, ya que hay demasiados casos en esa situación que pueden estar influenciando el análisis.

```{r}
datos_sent_fil<- datos_sent[datos_sent$polarity!=0,]


modelo_like_fil <- lm(datos_sent_fil$polarity~datos_sent_fil$likes_count_fb)

summary(modelo_like_fil)
```

En este caso tenemos un p-valor=0.01786 < 0.05, lo que nos llevaría a rechazar la hipótesis nula.
...
Ahora lo hacemos para comparar la variable categórica 'type' con el resto de variables continuas
...
El p-valor es muy alto , no podemos asegurar que estén correlacionadas.

Dado que el nº de posts con polaridad=0 es muy alto vamos a ver si eliminandolos obtenemos otro resultado. Quizá en análisis de sentimientos no ha sido correcto y por eso hay tantos valores con polaridad=0.

```{r}

datos_status_fil <- datos_status[datos_status$polarity!=0,]

modelo_eng_fil <- lm(polarity ~ eng_level, data=datos_status_fil)
summary(modelo_eng_fil)

```

```{r}
ggplot(data = datos_status_fil) + 
   geom_boxplot(mapping = aes(x = eng_level, y =   polarity))+
  xlab("nivel engagement") + ylab("polaridad") +
...
Aunque visualmente podría parecer que de esta forma si se cumple lo que sospechábamos (el nivel de engagement está relacionado con la polaridad) el p-valor obtenido es de 0.63, lo que nos lleva a rechazar la hipótesis nula.
...
ggplot(data = datos_status_fil1) + 
  ...
  geom_hline(yintercept = 0.62, colour="red")
```


En el caso de la subjetividad ocurre lo mismo, el p-valor no es lo suficientemente bajo como para rechzar la hipótesis nula, por lo que podemos concluir que no están correlacionadas.
